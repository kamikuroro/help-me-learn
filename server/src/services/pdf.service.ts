...
export async function extractTOC(filePath: string): Promise<{
  ...
  const script = [
    ...
    '    "chapters": []',
    '}',
    '# Filter to level-1 entries only (actual chapters, not subsections)',
    'top_level = [(i, e) for i, e in enumerate(toc) if e[0] == 1]',
    '# If no level-1 entries, fall back to level-2',
    'if not top_level:',
    '    top_level = [(i, e) for i, e in enumerate(toc) if e[0] == 2]',
    '# If still none, use all entries',
    'if not top_level:',
    '    top_level = list(enumerate(toc))',
    'for idx, (orig_i, entry) in enumerate(top_level):',
    '    level, title, page = entry',
    '    # Page end is the start of the next top-level entry minus 1, or total pages',
    '    if idx + 1 < len(top_level):',
    '        page_end = top_level[idx + 1][1][2] - 1',
    '    else:',
    '        page_end = len(doc)',
